...
- Starts a comparison session from popup input with two explicit URLs:
  - Staging URL
  - Production URL
- Keeps user control of final URLs while offering lightweight assist actions:
  - Use current tab as STG
  - Use current tab as PROD
  - Generate paired URL from the opposite side + mapping hints
  - Recent URL pair suggestions
...
- `tabs`: manage comparison tabs, query active tab URL for popup assist, and navigate paired pages.
- `windows`: create/position two normal comparison windows.
- `scripting`: inject compare agent scripts into compared tabs.
- `storage`: persist/recover active session + popup URL hints/recent pairs.
