...
import { getSession, setSession, clearSession, recordRecentPair } from '../shared/storage.js';
import { generateSessionId } from '../shared/utils.js';
import { mapUrlToOtherSide } from './url-mapper.js';
...
export async function startSession(payload) {
  ...
  const parsedStaging = new URL(stagingUrl);
  const parsedProduction = new URL(productionUrl);
  const leftUrl = parsedStaging.toString();
  const rightUrl = parsedProduction.toString();
  ...
  const session = {
    ...
    stagingBaseUrl: `${parsedStaging.origin}/`,
    productionBaseUrl: `${parsedProduction.origin}/`,
  ...
  await recordRecentPair(leftUrl, rightUrl);
...
export async function reopenMissingSide() {
  ...
  const leftUrl = session.lastKnownLeftUrl || session.stagingBaseUrl;
  const rightUrl = session.lastKnownRightUrl || session.productionBaseUrl;
